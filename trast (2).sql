...
(0, 'Pentru care dintre faptele de mai jos se aplică măsura reţinerii permisului de conducere?', 'pentru nerespectarea semnalelor, indicaţiilor şi dispoziţiilor poliţistului rutier aflat în exercitarea atribuţiilor de serviciu;', 'pentru nerespectarea dispoziţiilor legale referitoare la oprire şi staţionare;', 'pentru încălcarea normelor legale referitoare la semnalizarea vehiculelor.', 'A', '', 1),
...
--
DELIMITER $$
CREATE TRIGGER `progress_insert` AFTER INSERT ON `users` FOR EACH ROW insert into user_progress VALUES(NEW.id,0,0,0)
$$
DELIMITER ;
...
CREATE TABLE `user_progress` (
  `user_id` int(11) NOT NULL,
  `intrebari_invatare` int(11) NOT NULL,
  `chestionare_t` int(11) NOT NULL,
  `chestionare_c` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
